...
namespace YouTube_Downloader
{
    public static class YoutubeVideoUtil
    {
        ...
        {
            var v = GetListFromYoutube(youtubeUrl);
        ...
        private static VideoInfo SortedYoutubeObject(IEnumerable<VideoInfo> v)
        {
            ...
            var video = v
                .FirstOrDefault(
                    info => (info.VideoType == VideoType.Mp4) && (info.Resolution == Configs.VideoResolution));
            ...
            if (video == null)
            {
                ...
                SystemMessage.WriteConsoleMessage(
                    $"{Environment.NewLine}Could not find a {Configs.VideoResolution}p version of the video you where looking for. Downloading a {video.Resolution}p version {Environment.NewLine}",
                    ColorEnum.Yellow, false, false);
            }
            return video;
        }
    }
